(() => {
  ...
  var MAX_CHARS = 12e3;
  var MIN_JSONLD_BODY = 400;
  var MIN_READABILITY_CHARS = 200;
  ...
  function isArticleLike(obj) {
    return ldTypes(obj).some((t) => {
      ...
      return l.includes("newsarticle") || l === "article" || l.includes("blogposting") || l.includes("reportagearticle");
    });
  }
  ...
  function getArticleBody(obj) {
    ...
    if (Array.isArray(b)) {
      return b.map((x) => typeof x === "string" ? x : textField(x)).filter(Boolean).join("\n\n").trim();
    }
    return "";
  }
  function getHeadline(obj) {
    return textField(obj?.headline) || textField(obj?.name) || textField(obj?.title) || "";
  }
  ...
  function tryJsonLd() {
    ...
    for (const obj of objs) {
      ...
      const body2 = getArticleBody(obj);
      if (body2.length > bestLen) {
        bestLen = body2.length;
        best = obj;
      }
    }
  ...
  function tryReadability() {
    try {
      ...
      const raw = [article.title, article.textContent].filter(Boolean).join("\n\n") || article.textContent || "";
